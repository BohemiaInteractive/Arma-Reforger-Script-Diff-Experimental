...
/*!
Curve class for Color values. Expected to use as a property (using Attribute) and filled from config edited in dedicated Curve dialog in Workbench. But you can also initialize the curve programmatically using InitSpline or InitCustomSpline.
...
class CurveColor: Managed
{
	...
	*/
	proto external void InitSpline(notnull array<float> knots, notnull array<ref Color> values, ECurveType type = ECurveType.Akima, ECurveEnds ends = ECurveEnds.Open, float paramMin = -float.MAX, float paramMax = float.MAX, float valueMin = -float.MAX, float valueMax = float.MAX);
	/*!
	...
	\param valueMax				The range for curve value clamping, infinite by default.
	*/
	proto external void InitCustomSpline(notnull array<float> knots, notnull array<ref Color> values, notnull array<Color> tangentsL, notnull array<Color> tangentsR, ECurveEnds ends = ECurveEnds.Open, float paramMin = -float.MAX, float paramMax = float.MAX, float valueMin = -float.MAX, float valueMax = float.MAX);
	/*!
	Compute value at x.
