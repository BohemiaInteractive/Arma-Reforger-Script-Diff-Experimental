...
/*!
Curve class for vector values. Expected to use as a property (using Attribute) and filled from config edited in dedicated Curve dialog in Workbench. But you can also initialize the curve programmatically using InitSpline or InitCustomSpline.
...
class CurveVector: Managed
{
	...
	*/
	proto external void InitSpline(notnull array<float> knots, notnull array<vector> values, ECurveType type = ECurveType.Akima, ECurveEnds ends = ECurveEnds.Open, float paramMin = -float.MAX, float paramMax = float.MAX, float valueMin = -float.MAX, float valueMax = float.MAX);
	/*!
	...
	\param valueMax				The range for curve value clamping, infinite by default.
	*/
	proto external void InitCustomSpline(notnull array<float> knots, notnull array<vector> values, notnull array<vector> tangentsL, notnull array<vector> tangentsR, ECurveEnds ends = ECurveEnds.Open, float paramMin = -float.MAX, float paramMax = float.MAX, float valueMin = -float.MAX, float valueMax = float.MAX);
	/*!
	Compute value at x.
