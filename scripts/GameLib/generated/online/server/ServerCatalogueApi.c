...
You can switch to different modes which will determine source
...
sealed class ServerCatalogueApi
{
	...
	*/
	...
	/*!
	Sets callback which will be invoked when available ping sites are fetched
	and measured ping RTT.
	\note If sites are already loaded then this invokes immediately when set.
	\warning If sites are not yet fetched then getter for its list will be empty
	         and servers will not provide any ping sites until loaded.
	*/
	static proto void SetOnPingSitesReady(notnull BackendCallback pCallback);
	/*!
	Sets simple method as callback which is invoked when ping is measured against
	available ping sites.
	\note Ping is measured in set interval which cannot be changed.
	      Measurement is performed automatically once at the beginning and then automatically
	      if enabled from gamecode (cannot be enabled via scripts).
	\warning Soft refresh and ping measurements are performed in different intervals
	         but callback from soft refresh can be used to also get updated values which
	         could be delayed compared to using this callback.
	*/
	static proto void SetOnPingSitesMeasured(func onMeasured);
	/*!
	Returns list of all available ping sites which can be used to measure regional pings.
	\note Callback set via SetOnPingSitesReady must first invoke OnSuccess before this list
	      can be accessed.
	*/
	static proto void GetAvailablePingSites(out array<ServerPingSite> pingSitesOut);
	/*!
