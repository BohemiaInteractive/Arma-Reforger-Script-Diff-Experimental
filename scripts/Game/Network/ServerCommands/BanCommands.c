...
class StateBackendCallback : BackendCallback
{
	...
	//------------------------------------------------------------------------------------------------
	// BackendCallbackFunc
	protected void OnSuccessCallback(BackendCallback cb = null)
	{
	...
	//------------------------------------------------------------------------------------------------
	// BackendCallbackFunc
	protected void OnErrorCallback(BackendCallback cb = null)
	{
		bool isTimeout;
		if (cb)
		{
			isTimeout = cb.GetHttpCode() == HttpCode.HTTP_CODE_408; // request timeout
			if (!isTimeout)
				m_eLastError = cb.GetApiCode();
		}
...
class BanCommand : ScrServerCommand
{
	...
	protected ScrServerCmdResult CreateBan(array<string> argv, int playerId)
	{
		...
		switch (idType)
		{
			case SCR_EPlayerIdType.EPIT_PLAYERID:
				...
				PrintFormat("Request Ban %1[%2] for %3 seconds. Reason: '%4'", m_sPlayerName, m_iBanPlayerId, m_iDuration, m_sBanReason, level: LogLevel.NORMAL);
			...
			case SCR_EPlayerIdType.EPIT_IDENTITY:
				...
				PrintFormat("Request Ban %1 for %2 seconds. Reason: '%3'", m_sPlayerName, m_iDuration, m_sBanReason, level: LogLevel.NORMAL);
	...
	protected ScrServerCmdResult RemoveBan(array<string> argv, int playerId)
	{
		...
		if (idType == SCR_EPlayerIdType.EPIT_USERNAME)
		{
			...
			PrintFormat("Requested unban of user with user name: '%1'", argv[SCR_EBanRemoveArgs.EBRA_ID], level: LogLevel.NORMAL);
		}
		else
		{
			success = m_BanApi.RemoveBans(m_Callback, {argv[SCR_EBanRemoveArgs.EBRA_ID]});
			PrintFormat("Requested unban of user with identity: '%1'", argv[SCR_EBanRemoveArgs.EBRA_ID], level: LogLevel.NORMAL);
		}
	...
	protected ScrServerCmdResult ListBansResult()
	{
		...
		array<OnlineBanListData> banList = {};
	...
	protected SCR_EPlayerIdType GetPlayerIdType(string id)
	{
		...
		if (SCR_StringHelper.IsFormat(SCR_EStringFormat.DIGITS_ONLY, id))
	...
	protected int GetPlayerIdFromName(string name)
	{
		...
		{
			if (playerManager.GetPlayerName(pid) == name)
				return pid;
		}
		...
		switch (m_Callback.m_eLastError)
		{
			...
			case EApiCode.EACODE_ERROR_NOT_FOUND:
				if (m_Callback.m_eLastMatchType == EStringMatchType.ESMT_EQUALS)
				{
					...
					PrintFormat("Retrying unban of user with user name: '%1'", m_sPlayerName, level: LogLevel.NORMAL);
					if (!success)
...
class KickCommand : ScrServerCommand
{
	...
	protected ScrServerCmdResult KickPlayer(notnull array<string> argv, int playerId = 0)
	{
		if (argv.Count() < 2)
			return ScrServerCmdResult(string.Empty, EServerCmdResultType.PARAMETERS);
		...
		PrintFormat("Kick %1[%2]", kickPlayerName, kickPlayerId, level: LogLevel.NORMAL);
