...
class SCR_RewindComponent : SCR_BaseGameModeComponent
{
	...
	bool CanRewind()
	{
		return GetGame().GetSaveGameManager().IsSavingEnabled();
	}
	...
	void CreateRewindPoint()
	{
		...
		GetGame().GetSaveGameManager().RequestSavePoint(ESaveGameType.AUTO, s_RewindPointName, ESaveGameRequestFlags.BLOCKING, new SaveGameOperationCallback(OnRewindPointCreated));
	}
	...
	override void OnPostInit(IEntity owner)
	{
		...
		const SaveGame save = manager.GetActiveSave();
	...
	{
		const SaveGameManager manager = GetGame().GetSaveGameManager();
		manager.GetSaves(manager.GetCurrentMissionResource(), new SaveGameObtainCallback(ProcessRemoveUnusedRewindPoints));
	}

	//------------------------------------------------------------------------------------------------
	protected void ProcessRemoveUnusedRewindPoints(bool success, array<SaveGame> saves)
	{
		const SaveGameManager manager = GetGame().GetSaveGameManager();
