...
class SCR_Tutorial_ShootingRange_SHOOT_PRONEClass: SCR_BaseTutorialStageClass
{
};

//------------------------------------------------------------------------------------------------
class SCR_Tutorial_ShootingRange_SHOOT_PRONE : SCR_BaseTutorialStage
{
	...
	override protected void Setup()
	{
		...
		PlayNarrativeCharacterStage("SHOOTINGRANGE_Instructor", 8);
	}
	...
	{	
		...
		if (!m_Player || m_Player.GetCharacterController().GetStance() != ECharacterStance.PRONE)
		{
			if (!m_bCommented)
			{
				m_bCommented = true;
				PlayNarrativeCharacterStage("SHOOTINGRANGE_Instructor", 17);
			}
	...
	//------------------------------------------------------------------------------------------------
	void ~SCR_Tutorial_ShootingRange_SHOOT_PRONE()
	{
		if (!m_aTargets || m_aTargets.IsEmpty())
			return;
