...
class SCR_Tutorial_LRS_Targets_CliffClass: SCR_BaseTutorialStageClass
{
};

//------------------------------------------------------------------------------------------------
class SCR_Tutorial_LRS_Targets_Cliff : SCR_BaseTutorialStage
{
	...
	override protected void Setup()
	{
		...
		m_Waypoint = RegisterWaypoint("WP_TARGETS_FURTHER", "", "TARGETCHAR");

		m_TutorialComponent.SetupTargets("TARGETS_CLIFF", m_aTargets, ETargetState.TARGET_UP, false);
		...
		PlayNarrativeCharacterStage("LONGRANGESHOOTING_Instructor", 7);
	}
	...
	//------------------------------------------------------------------------------------------------
	void ~SCR_Tutorial_LRS_Targets_Cliff()
	{
		if (!m_aTargets || m_aTargets.IsEmpty())
			return;
