[BaseContainerProps()]
class SCR_LoiterRandomItemData : SCR_LoiterItemDataBase
{
	...
	protected ref array<ResourceName> m_aItemPrefabs;
	...
	[Attribute(defvalue: "0", desc: "If true, then game will try use data about the animation owner which as deterministic seed for the selection of the item, thus ensuring that all clients see the same item.")]
	protected bool m_bSameItemForAllClients;

	//------------------------------------------------------------------------------------------------
	override array<IEntity> SpawnPrefabs(notnull IEntity animationOwner, int seed)
	{
		RandomGenerator rng = new RandomGenerator();
		if (m_bSameItemForAllClients)
			rng.SetSeed(seed);

		ResourceName prefab = m_aItemPrefabs[rng.RandInt(0, m_aItemPrefabs.Count())];
