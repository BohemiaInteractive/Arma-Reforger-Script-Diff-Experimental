...
class SCR_TaskSave
{
	...
	void Save(notnull SCR_Task task, notnull PersistenceSystem persistence)
	{
		...
		{
			UUID id = GetTaskExecutorId(asignee, persistence);
			if (!id.IsNull())
		...
		if (groupManager)
		{
			...
			{
				UUID id = persistence.GetId(groupManager.FindGroup(ownerGroup));
				if (!id.IsNull())
		...
		{
			UUID id = GetTaskExecutorId(owner, persistence);
			if (!id.IsNull() && !m_aAssignees.Contains(id))
				m_aAssignees.Insert(id);
		}
