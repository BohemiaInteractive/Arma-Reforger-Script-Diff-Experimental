...
class AIControlComponentSerializer : ScriptedComponentSerializer
{
	...
	{
		const SCR_ChimeraAIAgent agent = SCR_ChimeraAIAgent.Cast(SCR_AIUtils.GetAIAgent(owner));
		if (!agent)
	...
	{
		SCR_ChimeraAIAgent agent = SCR_ChimeraAIAgent.Cast(SCR_AIUtils.GetAIAgent(owner));
		if (!agent)
			return true;
