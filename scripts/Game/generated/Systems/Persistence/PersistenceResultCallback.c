...
class PersistenceResultCallback: PersistenceCallback
{
	...
	proto void PersistenceResultCallback(Managed context = null, PersistenceResultDelegate delegate = null);
	/*!
	...
	*/
	event protected void OnResult(Managed context, EPersistenceStatusCode statusCode, Managed result, bool isLast);

	//! Invoke the delegate assigned to this callback. Returns false if there is none. To be used from OnResult.
	proto external bool InvokeDelegate(Managed context, EPersistenceStatusCode statusCode, Managed result, bool isLast);
}
