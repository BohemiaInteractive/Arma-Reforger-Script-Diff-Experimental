...
class ScriptedComponentSerializer: PersistenceSerializerBase
{
	...
	*/
	...
	proto external protected ESerializeResult SerializeNative(notnull IEntity owner, notnull GenericComponent component, notnull SaveContext context, ESerializeMode mode = ESerializeMode.NATIVE);
	//! Native deserialization logic used by the game. Can be called by scripts before or after custom data or not all.
	proto external protected bool DeserializeNative(notnull IEntity owner, notnull GenericComponent component, notnull LoadContext context, ESerializeMode mode = ESerializeMode.NATIVE);
	...
	*/
	event protected ESerializeResult Serialize(notnull IEntity owner, notnull GenericComponent component, notnull SaveContext context) { return SerializeNative(owner, component, context); };
	/*!
	Custom implementation for deserialzing the component data from the load context
	\return True for success. False for data/state errors which will abort the process and use configured fault handling
	*/
	event protected bool Deserialize(notnull IEntity owner, notnull GenericComponent component, notnull LoadContext context) { return DeserializeNative(owner, component, context); };
}
