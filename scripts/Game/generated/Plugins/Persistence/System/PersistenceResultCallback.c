...
void PersistenceResultDelegate(EPersistenceStatusCode statusCode, Managed result, bool isLast, Managed context = null); typedef func PersistenceResultDelegate;
class PersistenceResultCallback: PersistenceCallback
{
	...
	\param result The result instance which must be casted into your expected type. Usually IEntity or custom script class.
				  Special case: If a specific id should have been fetched, but the status code is not OK, the result is the internal id it failed to fetch.
